...
import com.blanketcobblespawners.utils.gui.pokemonsettings.*
        ...
        when (clickedSlot) {
            ...
            53 -> { // Next Page Button
                ...
                if (clickedItem.item is PokemonItem) {
                    ...
                    val showFormsInGui = ConfigManager.configData.globalConfig.showFormsInGui

                    if (matchResult != null) {
                        val (name, form) = matchResult.destructured
                        speciesName = name
                        // **Modification: Always set the formName, even if showFormsInGui is false**
                        formName = if (form.equals("Normal", ignoreCase = true)) "Normal" else form
                    } else {
                        speciesName = clickedItemName
                        // **Modification: Set formName to "Normal" if no form is specified**
                        formName = "Normal"
                    }


                    val existingEntry = selectedPokemon.find {
                    ...
                    when (context.clickType) {
                        ClickType.LEFT -> {
                            if (existingEntry == null) {
                                ...
                                ConfigManager.addPokemonSpawnEntry(spawnerPos, newEntry)
                                player.sendMessage(
                                    Text.literal("Added $speciesName${if (formName != null) " ($formName)" else ""} to the spawner."),
                                    false
                                )
                            } else {
                                // Remove Pokémon from the spawner
                                ConfigManager.removePokemonSpawnEntry(spawnerPos, speciesName, formName)
                                player.sendMessage(
