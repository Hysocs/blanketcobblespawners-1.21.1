...
import com.blanketcobblespawners.utils.CommandRegistrar.toggleSpawnerVisibility
...
object SpawnerSettingsGui {
    ...
    private fun adjustSpawnerSetting(
        ...
        ConfigManager.updateSpawner(spawnerPos) { spawnerData ->
            when (setting) {
                "Spawn Timer" -> {
    ...
    private fun toggleSpawnerVisibility(context: InteractionContext, player: ServerPlayerEntity, spawnerPos: BlockPos, visible: Boolean) {
        ...
        val success = CommandRegistrar.toggleSpawnerVisibility(server, spawnerPos)
        if (success) {
